...
def multi_mse_loss(input, target):
    mses = []
    for i in range(len(target)):
        mse = 0

        weights = [1/(1+math.exp(-inp)) for inp in input[i]]
        weights = [w/sum(weights) for w in weights]

        for j in range(len(input[i])):
            mse += weights[j]*((float(j+1) - target[i].item()) ** 2)
        mses.append(mse)
    return sum(mses)/len(mses)
#    return torch.Tensor(sum(mses)/len(mses), device='cuda:0', grad_fn=<NllLossBackward>)
...
##            loss = criterion(pred, batch_y)
            loss = multi_mse_loss(pred, batch_y)            
            print("Loss: " + str(loss))
...
##    pred = [sum([math.log(m[i])*float(j+1)]) for m in model(x).cpu().data.numpy()]
    ...
    for i in range(len(target)):
        ...
        for j in range(len(input[i])):
            mse += weights[j]*((float(j+1) - target[i]) ** 2)
        mses.append(mse)

    acc = sum(mses) / len(pred)
    return acc
