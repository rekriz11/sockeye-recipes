...
def get_perplexities(sentences, lm):
    ...
    debug_output(sentences, perplexities, "ORIGINAL PERPLEXITIES")
        
    norm_perps = normalize(perplexities)
    debug_output(sentences, norm_perps, "NORMALIZED PERPLEXITIES")
    return norm_perps
...
def load_comp_preds(comp_pred_file, sentences):
    ...
    debug_output(sentences, scores, "ORIGINAL COMPLEXITIES")
    
    norm_scores = normalize(scores)
    debug_output(sentences, norm_scores, "NORMALIZED COMPLEXITIES")
    return norm_scores
...
def get_sims(complex_embeddings, embeddings, sentences):
    ...
    debug_output(sentences, similarities, "ORIGINAL SIMILARITIES")

    norm_sims = normalize(similarities)
    debug_output(sentences, norm_sims, "NORMALIZED SIMILARITIES")
    return norm_sims
...
def rank_candidates(sentences, perplexities, comp_preds, similarities, weights):
    ...
    for i in range(len(sentences)):
        ...
        for j in range(len(sentences[i])):
            ...
            print(str(score) + "\t" + " ".join(sentences[i][j]))
        print("\n")

        max_index = scores.index(max(scores))

        print(sentences[i][max_index])
        print("\n")
...
'''
python ~/sockeye-recipes/new_scripts/cluster_rerank/rerank_TEST.py \
...
1 0 0 \
~/sockeye-recipes/egs/pretrained_embeddings/output/tokens.100beam.top1reranked_TEST
'''
